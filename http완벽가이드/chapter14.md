...
SSL과 TLS의 가장 인기있는 오픈소스 프로젝트.

### 14.8.2 간단한 HTTPS 클라이언트

클라이언트와 서버는 SSL 커넥션을 맺고, 그 사이트 서버로부터 가져온 신원 정보를 출력하고, HTTP GET 요청을 보안 채널을 통해 보내고 HTTP 응답을 받아 그 응답을 출력한다.

OpenSSL를 이용하여 간단한 HTTPS 클라이언트를 구현할 수 있다.

### 14.8.3 우리는 단순한 OpenSSL 클라이언트 실행하기

## 14.9 프락시를 통한 보안 트래픽 터널링

클라이언트는 종종 그들을 대신하여 웹 서버에 접근해주는 웹 프락시 서버를 이용한다. 이 프락시는 방화벽 라우터가 HTTP 트래픽의 교환을 허락한 유일한 장치이며, 바이러스 검사나 기타 컨텐츠 제어를 수행할 것이다.

클라이언트가 서버로 보낼 데이터를 서버의 공개키로 암호화 하기 시작했다면, 프락시는 더이상 HTTP 헤더를 읽을 수 없다. HTTPS 터널링 프로토콜을 사용해서 클라이언트는 프락시에게 자신이 연결하고자 하는 안전한 호스트와 포트를 말해준 후 이 내용을 프락시가 읽을 수 잇도록 암호화 전의 평문으로 말해준다.

HTTP는 CONNECT라 불리는 확장 메서드를 사용하여 클라이언트와 서버가 안전하게 데이터를 오갈 수 있도록 터널을 만들 수 있게 요청한다.
